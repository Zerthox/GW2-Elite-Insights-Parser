...
internal class Dhuum : HallOfChains
{
    internal readonly MechanicGroup Mechanics = new([
            new PlayerDstHealthDamageHitMechanic(HatefulEphemera, new (Symbols.Square,Colors.LightOrange), "Golem.D", "Hateful Ephemera (Golem AoE dmg)","Golem Dmg", Sev1, 0),
            new MechanicGroup([
                new PlayerDstHealthDamageHitMechanic(ArcingAfflictionHit, new (Symbols.CircleOpen,Colors.Red), "Bomb dmg", "Arcing Affliction (Bomb) hit","Bomb dmg", Sev3, 0),
                new PlayerDstBuffApplyMechanic(ArcingAffliction, new (Symbols.Circle,Colors.Red), "Bomb", "Arcing Affliction (Bomb) application","Bomb", Sev1, 0),
                new PlayerDstBuffRemoveMechanic(ArcingAffliction, new (Symbols.Diamond,Colors.Red), "Bomb Trig","Arcing Affliction (Bomb) manualy triggered", "Bomb Triggered", Sev0, 0).UsingChecker((br, log) =>
                {
            ...
            new MechanicGroup([
                new PlayerDstHealthDamageHitMechanic(ConeSlash, new (Symbols.TriangleUp,Colors.DarkGreen), "Cone", "Boon ripping Cone Attack","Cone", Sev0, 0),
                new PlayerDstHealthDamageHitMechanic(CullDamage, new (Symbols.BowtieOpen,Colors.Teal), "Crack", "Cull (Fearing Fissures)","Cracks", Sev0, 0),
                new PlayerDstHealthDamageHitMechanic(PutridBomb, new (Symbols.Circle,Colors.DarkGreen), "Mark", "Necro Marks during Scythe attack","Necro Marks", Sev2, 0),
                new PlayerDstHealthDamageHitMechanic(CataclysmicCycle, new (Symbols.CircleOpen,Colors.LightOrange), "Suck dmg", "Damage when sucked to close to middle","Suck dmg", Sev1, 0),
                new MechanicGroup([
                    new PlayerDstHealthDamageHitMechanic(DeathMark, new (Symbols.Hexagon,Colors.LightOrange), "Dip", "Lesser Death Mark hit (Dip into ground)","Dip AoE", Sev0, 0),
                    new PlayerDstHealthDamageHitMechanic(GreaterDeathMark, new (Symbols.Circle,Colors.LightOrange), "KB dmg", "Knockback damage during Greater Deathmark (mid port)","Knockback dmg", Sev3, 0),
                ]),
                new PlayerDstHealthDamageHitMechanic(RendingSwipe, new (Symbols.TriangleLeft, Colors.LightOrange), "Enf.Swipe", "Hit by Dhuum's Enforcer Rending Swipe", "Rending Swipe Hit", Sev1, 0),
            ]),
            new MechanicGroup([
                new PlayerSrcPlayerDstBuffApplyMechanic(DhuumShacklesBuff, new (Symbols.Diamond,Colors.Teal), "Shackles","Soul Shackle (Tether) application", "Shackles", Sev0,10000),//  //also used for removal.
                new PlayerDstHealthDamageHitMechanic(DhuumShacklesHit, new (Symbols.DiamondOpen,Colors.Teal), "Shackles dmg", "Soul Shackle (Tether) dmg ticks","Shackles Dmg", Sev0, 0)
                    .UsingChecker((de,log) => de.HealthDamage > 0),
            ]),
            new MechanicGroup([
                new PlayerCastStartMechanic(DhuumEtherealSealInteract, new (Symbols.CircleOpen,Colors.Teal), "Eth.Seal.S","Started channeling an Ethereal Seal", "Ethereal Seal channeling", Sev1,0)
                    .UsingChecker((gie, log) => !gie.IsInterrupted),
                new PlayerCastEndMechanic(DhuumEtherealSealInteract, new (Symbols.Circle,Colors.Teal), "Eth.Seal.I","Succesfully interacted with an Ethereal Seal", "Ethereal Seal interacted", Sev0,0)
                    .UsingChecker((gie, log) => !gie.IsInterrupted),
                new PlayerCastEndMechanic(DhuumEtherealSealInteract, new (Symbols.CircleCross,Colors.Teal), "Eth.Seal.F","Failed to interact with an Ethereal Seal", "Ethereal Seal failed", Sev0,0)
                    .UsingChecker((gie, log) => gie.IsInterrupted),
            ]),
            new PlayerDstBuffApplyMechanic(Superspeed, new (Symbols.TriangleRight, Colors.Grey), "SupSpeed.Orb", "Gained Superspeed from Desmina (Walked over orb)", "Took Superspeed orb", Sev3, 0)
                .UsingChecker((bae, log) => bae.CreditedBy.IsSpecies(TargetID.DhuumDesmina)),
            new MechanicGroup([
                new PlayerDstBuffApplyMechanic(EchosPickup, new (Symbols.Square,Colors.Red), "Echo PU", "Picked up by Ender's Echo","Ender's Pick up", Sev0, 3000),
                new PlayerDstBuffRemoveMechanic(EchosPickup, new (Symbols.Square,Colors.Blue), "F Echo","Freed from Ender's Echo", "Freed from Echo", Sev0, 0)
                    .UsingChecker((br,log) => !log.CombatData.GetDeadEvents(br.To).Any(x => Math.Abs(x.Time - br.Time) <= 150)),
                new PlayerBreakbarDamageMechanic(new (Symbols.StarDiamond, Colors.White), "Echo.BrkDmg", "Breakbar damage done against Ender's Echo while a player is picked", "Breakbar Damage Ender's Echo", Sev1, 0, (log, a) => log.CombatData.GetBreakbarDamageData(a))
                    .UsingChecker((brae, log) => brae.To.IsSpecies(TargetID.EndersEcho) && EchoBreakbarMechanicChecker(log, brae.Time))
                    .UsingWeight(),
                new PlayerSrcBuffApplyMechanic([Fear, Taunt, Immobile, Slow], new (Symbols.StarDiamond, Colors.Red), "Echo.BrkCndApp1", "Applied Fear, Taunt, Immobile, Slow against Ender's Echo breakbar while a player is picked", "Strong Condition Breakbar Ender's Echo", Sev1, 0)
                    .UsingChecker((bae, log) => bae.To.IsSpecies(TargetID.EndersEcho) && EchoBreakbarMechanicChecker(log, bae.Time)),
                new PlayerSrcBuffApplyMechanic([Chilled, Blind, Weakness, Crippled], new (Symbols.StarDiamond, Colors.LightRed), "Echo.BrkCndApp2", "Applied Chilled, Blind, Weakness, Crippled against Ender's Echo breakbar while a player is picked", "Weak Condition Breakbar Ender's Echo", Sev1, 0)
                    .UsingChecker((bae, log) => bae.To.IsSpecies(TargetID.EndersEcho) && EchoBreakbarMechanicChecker(log, bae.Time)),
            ]),
            new PlayerSrcBuffApplyMechanic(DhuumsMessengerFixationBuff, new (Symbols.CircleOpenDot, Colors.Brown), "Mess Fix", "Fixated by Messenger", "Messenger Fixation", Sev2, 10)
